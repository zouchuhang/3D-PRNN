...
function getValLoss()
    local valnumberOfPasses = 37--24--88--opt.numPasses
    local valcount = 1
	...
	--print(i)
        ...
        for t = 1, maxLen - 1 do
            ...
            --local s_in = sMat[{{},{},{t}}]:squeeze(3)
            --local s_target = sMat[{{},{},{t+1}}]:squeeze(3)
            local cond_context = voxMat
            -- model 
       output_y[t], lstm_c_h1[t], lstm_h_h1[t],
            lstm_c_h2[t], lstm_h_h2[t], lstm_c_h3[t], lstm_h_h3[t], depth_fea, rot_pred[t]--, s_pred[t]
        = unpack(clones.rnn_core[t]:forward({x_in:cuda(), rot_in:cuda(), cond_context:cuda(), lstm_c_h1[t-1], lstm_h_h1[t-1],
...
function feval(x)
    ...
    for i=1, numberOfPasses do
       ...
       -- print(count)
        ...
        output_y[t], lstm_c_h1[t], lstm_h_h1[t],
            lstm_c_h2[t], lstm_h_h2[t], lstm_c_h3[t], lstm_h_h3[t], depth_fea, rot_pred[t]
        = unpack(clones.rnn_core[t]:forward({x_in:cuda(), rot_in:cuda(), cond_context:cuda(),
        ...
        local ddepth_fea = torch.zeros(sampleSize, 32):cuda()
        for t = maxLen - 1, 1, -1 do
            ...
            --local s_in = sMat[{{},{},{t}}]:squeeze()
            --local s_target = sMat[{{},{},{t+1}}]:squeeze()
            local cond_context = voxMat
	    ...
	    grad_crit_r:clamp(-100,100)
            --grad_crit_s:clamp(-100,100)
            -- model
           _x,_r,_c, dlstm_c_h1, dlstm_h_h1,
            dlstm_c_h2, dlstm_h_h2, dlstm_c_h3, dlstm_h_h3 = unpack(clones.rnn_core[t]:backward({x_in:cuda(),rot_in:cuda(), cond_context:cuda(),
          
      lstm_c_h1[t-1], lstm_h_h1[t-1],
                 lstm_c_h2[t-1], lstm_h_h2[t-1], lstm_c_h3[t-1], lstm_h_h3[t-1]},
                 {grad_crit, dlstm_c_h1, dlstm_h_h1,
                  dlstm_c_h2, dlstm_h_h2, dlstm_c_h3, dlstm_h_h3, ddepth_fea, grad_crit_r}))
            -- clip gradients
            --dh1_w:clamp(-10,10)
...
for i = 1, iterations do
    ...
    if i % 20 == 0 then
        ...
        --vallosses_s[#vallosses_s + 1] = valLoss3
  	...
  	--print(string.format("validation sym loss = %6.8f", valLoss3))
        if minValLoss > valLoss then
            minValLoss = valLoss
        
            params_save = params:clone()
            nn.utils.recursiveType(params_save,'torch.DoubleTensor')
            torch.save("./model/model_param_mn_depth_tr.t7", params_save:double())
            model_save = model.rnn_core:clone()
            nn.utils.recursiveType(model_save,'torch.DoubleTensor')
            torch.save("./model/model_mn_depth_tr.t7", model_save:double())
            print("------- Model Saved --------")
        end
        losses[#losses + 1] = loss[1]
        --torch.save("losses_full_nyu_trval.t7", losses)
        --torch.save("vallosses_full_nyu_trval.t7", vallosses)
        --torch.save("vallosses_r_full_nyu_trval.t7", vallosses_r)
        --torch.save("vallosses_s_full.t7", vallosses_s)
    end
end
