...
require 'nngraph'
require 'optim'
local LSTMH1 = require 'LSTMH1_depth'
local LSTMHN = require 'LSTMHN_depth'
require 'yHatMat2'
...
valshapedata = valdataFile:readObject()
...
--model.s_criterion = nn.BCECriterion():cuda()
...
local w_vector = VOXnet.voxae()(input_context)

local h1 = LSTMH1.lstm(opt.inputSize, opt.hiddenSize)({input_xin, input_rin, w_vector, input_lstm_h1_c, input_lstm_h1_h})
local h1_c = nn.SelectTable(1)(h1)
local h1_h = nn.SelectTable(2)(h1)
local h2 = LSTMHN.lstm(opt.inputSize, opt.hiddenSize)({input_xin, input_rin, w_vector, h1_h, input_lstm_h2_c, input_lstm_h2_h})
local h2_c = nn.SelectTable(1)(h2)
local h2_h = nn.SelectTable(2)(h2)
local h3 = LSTMHN.lstm(opt.inputSize, opt.hiddenSize)({input_xin, input_rin, w_vector, h2_h, input_lstm_h3_c, input_lstm_h3_h})
...
local rot_l3 = nn.Linear(64, 32)(rot_l2)
local rot_res = nn.Linear(32, 1)(rot_l3)
